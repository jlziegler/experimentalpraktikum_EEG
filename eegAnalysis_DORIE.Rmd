...
## Augenbewegungen
```{r ICA augen, echo=T}
ICA_comp_eog <- ar_eogcor(decomp = dor02_ICA, 
                          data = dor02_epo, 
...
## Muskelartefakte
```{r}
ICA_comp_mus <- ar_acf(dor02_ICA)
```

## Channel ICA
```{r}
ICA_comp_chn <- ar_chanfoc(dor02_ICA)
```

## Trial ICA
```{r}
ICA_comp_trl <- ar_trialfoc(dor02_ICA)
```
...
## remove components
```{r}
dor02_cln <- apply_ica(data = dor02_epo, decomp = dor02_ICA, comps = ICA_comp_rem)
```
...
```{r}
dor02_cln %>% 
  select_epochs(epoch_no = dor02_cln$epochs$epoch
                [dor02_cln$epochs$epoch_labels == "ouStan" 
                  | dor02_cln$epochs$epoch_labels == "uoDev"]) %>% # mehrere Epochen!
  plot_timecourse("FCz", colour = "epoch_labels") + 
  scale_y_reverse()

```
- Idee für Skript von Plots: am Anfang Vektor erstellen, in dem die zu vergleichenden Conditions drin sind: ```epoch_comparisons <- c("ouStan", "uoDev")```, dann in Plot call bei Selektion: ```dor02_cln$epochs$epoch_labels==epoch_comparisons[2]``` -> ist das wirklich notwendig?
